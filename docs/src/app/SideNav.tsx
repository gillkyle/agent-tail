...
function TypedSymbol() {
	...
	useEffect(() => {
		...
		const timer = setTimeout(() => set_count((c) => c + 1), 150);
...
export function SideNav() {
	...
	const links: (
		| { href: string; label: string; items?: { id: string; text: string }[] }
		| { section: string }
	)[] = [
		{ href: "/", label: "Overview" },
		{
	...
	return (
		<nav className="side-nav">
			...
			<div className="nav-meta">
				<a
					href="https://github.com/gillkyle/agent-tail"
					target="_blank"
			...
			<div className="nav-original-docs">
				Original docs via{" "}
				<a
