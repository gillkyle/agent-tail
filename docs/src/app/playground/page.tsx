...
function BasicLevels() {
	return (
		<div className="playground-section">
			...
			<p>
				Each button fires the corresponding <code>console.*</code> method.
			</p>
...
function ApiSimulation() {
	...
	function simulateRequest(method: string) {
		...
		setLoading(method);
		console.info(`[api] ${method} ${endpoint} started (req:${requestId})`);
		...
		<div className="playground-section">
			...
			<div className="playground-btn-row">
				...
				<button
					className="playground-btn"
					onClick={() => {
						["GET", "POST", "PUT", "DELETE"].forEach((m) => simulateRequest(m));
					}}
				>
					Fire all
				</button>
	...
	return (
		<div className="playground-section">
			...
			<p>
				...
				<code>unhandledrejection</code> — caught by <code>captureErrors</code>{" "}
				and <code>captureRejections</code>.
			</p>
			<div className="playground-btn-row">
				<button
					className="playground-btn level-error"
		...
		<div className="playground-section">
			...
			<div className="playground-btn-row">
				...
				<button
					className="playground-btn"
					onClick={() => {
						console.debug(`[perf] component render #${renderCount.current}`);
					}}
				>
					Log render count
				</button>
				...
				<button
					...
					onClick={() => {
						if (!formData.name || !formData.email) {
							...
							setTimeout(() => {
								console.log(
									`[api] POST /api/users responded 201 in ${randomMs()}ms`,
								);
					...
					onClick={() => {
						...
						setTimeout(() => {
							...
							setTimeout(() => {
								...
								console.info(
									"[auth] login successful — redirecting to /dashboard",
								);
							}, 300);
						}, 500);
					}}
					...
					onClick={() => {
						...
						setTimeout(() => {
							...
							setTimeout(() => {
								...
								console.warn(
									"[auth] account locked for 5 minutes after 3 more attempts",
								);
							}, 200);
						}, 400);
					}}
				...
				<button
					...
					onClick={() => {
						...
						setTimeout(() => {
							...
							setTimeout(() => {
								console.log(
									"[payment] charge authorized (txn: ch_" + randomId() + ")",
								);
								console.log(
									"[checkout] order confirmed: ORD-" + randomId().toUpperCase(),
								);
								console.info(
					...
					onClick={() => {
						...
						setTimeout(() => {
							...
							setTimeout(() => {
								...
								setTimeout(() => {
									console.warn(
										"[deploy] health check: /api/search responded slowly (2.4s)",
									);
									console.log("[deploy] health check: 11/12 services healthy");
									console.warn(
										"[deploy] deploying with degraded search service",
									);
		...
		<>
			<article className="article playground-article">
				...
				<div className="playground-hint">
					...
					Refer to the agent-tail playground bar at the bottom of the screen
					which shows browser console output from this page, press buttons below
					to demo.
				</div>
